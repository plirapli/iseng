...
<body class="min-vh-100 d-flex flex-column">
  ...
  <div class="container col-md-6 py-4">
    <h2 class="mb-4 text-center">Edit Data Mahasiswa Baru</h2>
    <div class="p-5 bg-light rounded-3">
      <form action="./controller/query_edit.php?nim=<?= $data["nim"] ?>" method="POST">
        <h2>Form</h2>
        <div class="row mb-3">
          <label for="inputNama" class="fw-bold col-sm-2 col-form-label">Nama</label>
          <div class="col-sm-10">
            <input type="text" value="<?= $data["nama"] ?>" class="form-control" name="nama" id="inputNama" placeholder="Nama Lengkap">
          </div>
        </div>
        <div class="row mb-3">
          ...
          <div class="col-sm-10">
            <select name="fakultas" class="form-select">
              <?php foreach ($fakultas as $fak) : ?>
                <option value="<?= $fak["id_fakultas"] ?>" <?= $data["id_fakultas"] == $fak["id_fakultas"] ? "selected" : "" ?>>
                  <?= $fak["fakultas"] ?>
                </option>
              <?php endforeach ?>
            </select>
          ...
          <div class="col-sm-10">
            <?php for ($i = 2019; $i < 2022; $i++) : ?>
              <div class="form-check form-check-inline">
                <input class="form-check-input" type="radio" name="angkatan" id="<?= $i ?>" value="<?= $i ?>" <?= $data["angkatan"] == "$i" ? "checked" : "" ?>>
